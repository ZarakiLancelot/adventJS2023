// Santa 🎅 está probando su nuevo trineo eléctrico, el CyberReindeer, en una carretera
...
// una barrera cerrada, se detiene hasta que la barrera se abra. Si está abierta, la
...
function cyberReindeer(road, time) {
  ...
  for (let iteration = 1; iteration < time; iteration++) {
    if (iteration === 5) {
      road = road.replaceAll(closedBarrier, openedBarrier);
    }

    const regexMatch = road.match(/S[\*\.]/);

    if (regexMatch) {
      const [match] = regexMatch;
      road = road.replace(match, lastPoint + "S");
      lastPoint = match[1];
    }
